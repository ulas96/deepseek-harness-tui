...
theus is a Rust workspace with two crates:
...
- **dsh-harness-client** - the protocol client library (zero UI dependencies). The design twin of the harness's
  ...
  same layering.
- **theus** - the ratatui application (lib + bin).

## The runtime ownership model

theus owns the runtime subprocess for its whole life. Launch specs mirror the harness's example-launch resolver:
...
For an unconfigured `theus` invocation, the checkout is discovered from the current directory tree or common
...
The child environment is the parent environment verbatim plus mode-specific entries: theus's baseline credential
policy is pass-through, so DEEPSEEK_API_KEY / DEEPSEEK_BASE_URL reach the runtime unchanged. The interactive
'/provider add' flow adds a second, opt-in channel on top of that baseline: a route's API key, typed into the TUI's
provider-onboarding form, is sent once as 'credentialValue' on the 'provider/discover' / 'provider/add' JSON-RPC
calls for the runtime to persist to the owner-only '~/.dsh/.credentials.yaml' (route metadata goes to
'~/.dsh/settings.yaml'); theus never echoes, logs, or reads the value back, and it never crosses the wire outside
those two request bodies. stdout stays reserved for JSON-RPC frames - theus's shipped cordis.yml mounts no stdout
logger, and the client records non-JSON stdout lines as protocol violations that the UI renders loudly.
...
- **protocol** - the named request/result/notification shapes from '@deepseek-ai/dsh-sdk-protocol/types', including
  catalog, session-local model selection, provider discovery/add, and persisted session list/resume.
...
- **commands** - pure slash-command parsing. Commands never cross the model boundary; a doubled leading slash is the
  explicit literal escape.
- **ui** - ratatui rendering: transcript windowing (unicode-width wrapping + scroll), status bar (session id,
  running/idle, provider/model/effort, cwd@branch, elapsed, queued badge), input pane, and command overlays. Pure function of UiState, so
...
- **app** - the event loop: notification stream + keyboard thread + redraw tick; Enter enqueues (the fast RPC), the
  receipt-to-idle transition is observed from the stream; queued prompts fire on idle; slash-command pickers/forms
  call the extended management protocol; resumed events are folded through the same pure event mapper as live events;
  Ctrl+C confirms mid-turn because quitting abandons the turn by tearing down the runtime.
...
opt-in because it requires a prepared DeepSeek Harness checkout. theus never runs the harness's own test suite - it
is a consumer, not the monorepo.

## Mapping to the removal note's four conditions

1. **A named product or deployment** - theus is the named product with its own repository, CLI, and shipped cordis.yml.
...
- conservative XML-wrapper parsing for human-readable fallback output - theus renders committed assistant text
